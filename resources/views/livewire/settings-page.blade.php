        ...
        <div class="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 space-y-4">
            ...
            <div>
                ...
                <div class="mt-2 flex items-center gap-4">
                    ...
                    <label class="inline-flex items-center px-3 py-2 rounded-md border border-gray-300 dark:border-gray-600 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                        {{ __('Change') }}
                        <input type="file" wire:model="logoUpload" accept="image/*" class="hidden">
                    </label>

                    @if ($logo && ! $logoUpload)
                        <button type="button" wire:click="removeLogo" wire:confirm="{{ __('Remove the logo?') }}" class="text-sm text-red-600 dark:text-red-400 hover:underline">
                            {{ __('Remove') }}
                        </button>
                    @endif
                </div>
                <p class="mt-1 text-xs text-gray-400 dark:text-gray-500" wire:loading wire:target="logoUpload">{{ __('Uploading…') }}</p>
                @error('logoUpload') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
            </div>
